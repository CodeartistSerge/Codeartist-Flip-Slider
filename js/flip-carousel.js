...
$.fn.flipcarousel = function(options) {
    ...
    var ops = $.extend({
        ...
        //structural elements
    var $container = $('<div class="flip-carousel" style="min-height: ' + ops.min_height + '">'),
        $card = $('<div class="card">'),
        $faces = $('<div class="face front"></div><div class="face back"></div>'),
        /*$face_front = $('<div class="face front"></div>'),
        $face_back = $('<div class="face back"></div>'),*/
    ...
    function build(){ //make base html structure 
       ...
       /* $card.append($face_front);
        ...
        $arrowright.click(function(){
            if(i == pages.length-1) i = -1; // 0 will be next slide
            /*if(!disabled) */go(i+1); // flip is always available because it is infinite
        });
    ...
    function place(cells, face){ //place content into div.face (into either .front or .back) 
        for(c=0; c < ops.itemsperpage; c++){
            ...
            $card.eq(c).find(face).append($container.find('.controls.dots').last().clone());
        };
    }
